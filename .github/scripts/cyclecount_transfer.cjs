...
// PiMS Cycle Count transfer, protocol 1. Node.js 20+; no npm dependencies.
...
const MAX_PARTS = 80;
const EVENT_TYPE = 'pims-cyclecount-chunk';
const BRANCH_PREFIX = 'pims-cyclecount-transfer/';
const STAGING = '.cyclecount-transfer';
const DATA_PATH = 'data/cyclecount.json';
const ARRAY_FIELDS = ['summary', 'overallCurve', 'scenarioCurves', 'items', 'selections', 'impact', 'matrix'];
...
function validatePacket(p) {
  ...
  check(typeof p.chunk === 'string' && p.chunk.length === expected,
    'This part has the wrong length. Check CycleCountChunks and the loop index.');
...
function validateData(data, exportedAtUtc) {
  ...
  const required = {
    summary: ['scenarioId', 'scenario', 'coverageTarget', 'selectedItems', 'mandatoryItems', 'exposureTitle', 'exposureText', 'kneeRank', 'kneeCoverage'],
    overallCurve: ['rank', 'item', 'cumulativeCoverage', 'isKneePoint', 'kneeRank', 'kneeCoverage'],
    scenarioCurves: ['scenarioId', 'scenario', 'rank', 'cumulativeCoverage', 'kneeRank', 'kneeCoverage'],
    items: ['item', 'description', 'priorityRank', 'priorityIndex', 'mainDriver'],
    selections: ['scenarioId', 'coverageTarget', 'item'],
    impact: ['scenarioId', 'coverageTarget', 'impactType', 'sortOrder', 'itemCount'],
    matrix: ['scenarioId', 'coverageTarget', 'selectionStatus', 'minMaxAction', 'itemCount']
  };
  // DAX SELECTCOLUMNS aliases arrive as [fieldName] through Power BI.
  for (const field of ARRAY_FIELDS) {
    for (const row of data[field]) {
      for (const key of required[field]) {
        check(Object.hasOwn(row, key) || Object.hasOwn(row, `[${key}]`),
          `${field} is missing ${key}. Enable Include nulls in the query action.`);
      }
    }
  }
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      const response = await fetch(this.base + path, {
        ...
        headers: {Authorization: `Bearer ${this.token}`,
          ...
          'User-Agent': 'pims-cyclecount-transfer'},
...
async function currentData(store) {
  ...
  if (current) {
    check(object(current.value), 'Existing data/cyclecount.json is not an object.');
...
async function receive(store, packet, retry = false) {
  ...
  if (!manifest) {
    const value = {...metadata(p), producer: 'pims-cyclecount-chunks-v1', createdAtUtc: new Date().toISOString()};
    await store.putFile(manifestPath, jsonBytes(value), branch, undefined, 'Start Cycle Count transfer');
    manifest = {value};
  }
  check(manifest.value.producer === 'pims-cyclecount-chunks-v1', 'Unrecognized staging manifest.');
...
if (existing) {
  ...
  const saved = await store.putFile(path, jsonBytes(record), branch, undefined,
    `Receive Cycle Count part ${p.partNumber} of ${p.partCount}`);
...
async function finalize(store, packet, sourceSha, retry = false) {
  ...
  check(manifest && manifest.value.producer === 'pims-cyclecount-chunks-v1', 'Missing staging manifest.');
  assertMetadata(manifest.value, p);
  const records = [];
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      const saved = await store.putFile(DATA_PATH, result.bytes, 'main', current?.sha,
        `Update Cycle Count data ${p.exportedAtUtc}`);
      return {applied: true, commitSha: saved.commit.sha, sha256: result.sha256,
        rows: result.rows, status: 'The complete, validated Cycle Count file was committed to main.'};
    } catch (e) {
...
async function cleanup(store, now = Date.now()) {
  ...
  for (const branch of await store.matchingBranches()) {
    ...
    if (!manifest || manifest.value.producer !== 'pims-cyclecount-chunks-v1' ||
...
function selfTest() {
  const data = {schemaVersion: 1, exportedAtUtc: '2026-09-13T12:00:00.1234567Z',
    summary: [{'[scenarioId]': 1, '[scenario]': 'Baseline', '[coverageTarget]': 40,
      '[selectedItems]': 1, '[mandatoryItems]': 0, '[exposureTitle]': 'No Mandatory Constraint',
      '[exposureText]': '—', '[kneeRank]': 1, '[kneeCoverage]': 1}],
    overallCurve: [{rank: 1, item: 'Sample', cumulativeCoverage: 1, isKneePoint: 1, kneeRank: 1, kneeCoverage: 1}],
    scenarioCurves: [{scenarioId: 1, scenario: 'Baseline', rank: 1, cumulativeCoverage: 1, kneeRank: 1, kneeCoverage: 1}],
    items: [{item: 'Sample', description: 'Sample ₹é🚆', priorityRank: 1, priorityIndex: 1, mainDriver: null}],
    selections: [{scenarioId: 1, coverageTarget: 40, item: 'Sample'}],
    impact: [{scenarioId: 1, coverageTarget: 40, impactType: 'Common with Baseline', sortOrder: 1, itemCount: 1}],
    matrix: [{scenarioId: 1, coverageTarget: 40, selectionStatus: 'Selected', minMaxAction: 'Review Required', itemCount: 1}],
...
async function main() {
  ...
  if (mode === 'receive') {
    ...
    const result = await finalize(store, event.client_payload, process.env.CYCLECOUNT_SOURCE_SHA,
